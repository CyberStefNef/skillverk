...
func TestInstallAndForeignPreservation(t *testing.T) {
	root := tempDir(t)
	codex := filepath.Join(root, executableName("codex"))
...
func TestProviderCommandUsesInteractivePrompt(t *testing.T) {
	...
	for _, provider := range []string{"codex", "claude"} {
		path := filepath.Join(root, executableName(provider))
	...
	_ = os.Remove(filepath.Join(root, executableName("codex")))
...
func TestAutomaticProviderSelectionAndModelOverride(t *testing.T) {
	...
	executable := func(name string) {
		t.Helper()
		if e := os.WriteFile(filepath.Join(root, executableName(name)), []byte("#!/bin/sh\nexit 0\n"), 0755); e != nil {
			t.Fatal(e)
		}
	}
	executable("claude")
