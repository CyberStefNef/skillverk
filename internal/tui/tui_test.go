...
func TestSourceIsTypedInThePane(t *testing.T) {
	a, _ := fixture(t)
	source := writeSource(t, filepath.Join(tempDir(t), "more"), "gamma")
	press(t, a, "m")
	focusOn(t, a, "Import skills")
	press(t, a, "enter")
...
func TestImportSelectsWithoutActivating(t *testing.T) {
	a, repo := fixture(t)
	source := writeSource(t, filepath.Join(tempDir(t), "more"), "gamma", "delta")
...
func TestSkippedSourceEntriesStayReviewable(t *testing.T) {
	a, _ := fixture(t)
	dir := writeSource(t, filepath.Join(tempDir(t), "mixed"), "gamma")
...
func TestCancellingALoadIgnoresLateResults(t *testing.T) {
	...
	c, err := library.OpenCollection(writeSource(t, filepath.Join(tempDir(t), "late"), "gamma"))
	if err != nil {
		t.Fatal(err)
	}
	defer c.Close()
...
func TestAdoptedSkillsNameWhereTheyCameFrom(t *testing.T) {
	for _, c := range []struct{ source, want string }{
		...
		if got := adoptedFrom(c.source); got != filepath.FromSlash(c.want) {
...
func TestAdoptedSkillsExplainWhyTheyCannotBeChecked(t *testing.T) {
	...
	if !strings.Contains(detail, filepath.FromSlash("/home/s/work/pdx-cif")) || !strings.Contains(detail, "--replace") {
...
func TestSourceLocationsAreCompletePaths(t *testing.T) {
	for _, c := range []struct{ source, subpath, want string }{
		...
		if got := origin(library.Entry{Source: c.source, Subpath: c.subpath}); got != filepath.FromSlash(c.want) {
