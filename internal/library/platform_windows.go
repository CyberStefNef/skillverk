...
// A staged relative target may not exist yet, so os.Symlink cannot infer that
// it is a directory. The link must keep that type when moved into place.
func relativeDirectoryLink(target, path string) error {
	p, err := windows.UTF16PtrFromString(path)
	if err != nil {
		return err
	}
	...
	flags := uint32(windows.SYMBOLIC_LINK_FLAG_DIRECTORY | allowUnprivilegedCreate)
	err = windows.CreateSymbolicLink(p, t, flags)
	if errors.Is(err, windows.ERROR_INVALID_PARAMETER) {
		err = windows.CreateSymbolicLink(p, t, windows.SYMBOLIC_LINK_FLAG_DIRECTORY)
	}
	return err
}
